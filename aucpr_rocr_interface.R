...
.define.environments <- function() {
    ...
    assign("aucpr", "prec.stop", envir=optional.arguments)
    ...
    assign("aucpr:prec.stop", 1, envir=default.values) 
...
.performance.aucpr <-
  function(predictions, labels, cutoffs, fp, tp, fn, tn,
           n.pos, n.neg, n.pos.pred, n.neg.pred,prec.stop) {
...
#          prec[1] = 0
      ...
      #There have to  be at least 2 points for interpolation of the auc
      if (!(length(rec[rec<=prec.stop]) < 2)) {
          prec <- prec[rec<=prec.stop]
          rec <- rec[rec<=prec.stop]
          
          auc <- interpolate.area(recalls=rec,precisions=prec)
          ans <- list( c(), auc)
      }else{
          ans <- list(c(),NA)
      }
